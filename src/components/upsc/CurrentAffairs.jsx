...
async function runFetchNews({ topic = "", page = 1, pageSize = 10 }) {
  ...
  // Get the array of articles based on topic, or combine all if no topic is given
  let articles = [];

  if (topic) {
    articles = data[topic.toLowerCase()] || [];
  } else {
  ...
  return paged.map((item, idx) => ({
    ...
    tags: [
      ...(topic ? [topic] : []),
      ...(item.title?.toLowerCase().includes("government") ? ["Government"] : []),
      ...(item.title?.toLowerCase().includes("policy") ? ["Policy"] : []),
      ...(item.title?.toLowerCase().includes("economy") ? ["Economy"] : []),
      ...(item.title?.toLowerCase().includes("environment") ? ["Environment"] : []),
      ...(item.title?.toLowerCase().includes("science") ? ["Science & Tech"] : []),
      ...(item.title?.toLowerCase().includes("technology") ? ["Science & Tech"] : []),
      ...(item.title?.toLowerCase().includes("international") ? ["International"] : []),
      ...(item.title?.toLowerCase().includes("india") ? ["India"] : []),
      ...(item.title?.toLowerCase().includes("governance") ? ["Governance"] : []),
      ...(item.title?.toLowerCase().includes("schemes") ? ["Schemes"] : []),
      ...(item.title?.toLowerCase().includes("social") ? ["Social Issues"] : []),
      ...(item.title?.toLowerCase().includes("upsc") ? ["UPSC"] : []),
    ],
    source: item.url,
